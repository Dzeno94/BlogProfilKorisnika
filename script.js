...
function renderBlogs(blogs) {
    ...
    if(selectedUser){
        blogsToDisplay=blogs.filter(blog=>blog.author==selectedUser.name);
    }
   
    for (var blog of blogsToDisplay) {
        ...
        span.innerHTML = `Author: <span style="font-weight:bold;"><a id="selected-user" href="user.html">${blog.author}</a</span>`;
...
function AddUserNameClickEventListener(e){
    var userLinks =document.querySelectorAll("#selected-user");
    userLinks.forEach( function(userLink){
        userLink.addEventListener("click",(e) => {
            e.preventDefault();
               const selectedUsername = e.target.innerHTML;
            selectedUser=users.find((user,index)=>user.name==selectedUsername);
            var sidebar= document.createElement("div");
            sidebar.classList.add("sidebar");
            var blogInput=document.querySelector(".add-blog-form");
            var searchBlogs=document.querySelector(".blog-input");
            searchBlogs.style.display="none";
            blogInput.style.display="none";
            var ProfileName= document.createElement("h2");
            ProfileName.innerHTML=selectedUser.name;
            var ProfileAdress=document.createElement("p");
            ProfileAdress.innerHTML=selectedUser.address;
            var ProfileEmail=document.createElement("p");
            ProfileEmail.innerHTML=selectedUser.email;
            var ProfileUsername=document.createElement("p");
            ProfileUsername.innerHTML=selectedUser.username;
                console.log("ovo je user",selectedUser)
                renderBlogs(allBlogs);
                
                sidebar.appendChild(ProfileName);
                sidebar.appendChild(ProfileAdress);
                sidebar.appendChild(ProfileEmail);
                sidebar.appendChild(ProfileUsername);
...
/*function displayUserProfile(){
    ...
    for (var user of users) {
        if(user.name===selectedUser.name)
            var name = document.getElementById('user-name-sidebar');
            var email = document.getElementById("user-email");
            var address = document.getElementById("user-adress");
            var username= document.getElementById("user-nickname");
                name.innerHTML = `User:${user.name}`;
               
            email.innerHTML=`Email:${user.email}`;
            address.innerHTML=`Adress:${user.address}`;
